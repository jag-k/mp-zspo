...
(function() {
  var InputFile = function(element) {
    ...
    this.preview = this.element.getElementsByClassName('file-upload__preview')[0];
  ...
  function updateInputLabelText(inputFile) {
    ...
    if(inputFile.input.files && inputFile.input.files.length < 1) {
      ...
      console.error(inputFile, inputFile.preview);
      if (inputFile.preview) {
        inputFile.preview.src = inputFile.input.src;
        inputFile.preview.removeClass("is-hidden");
      } else {
        inputFile.preview.addClass("is-hidden");
      }
    } else if(inputFile.multipleUpload && inputFile.input.files && inputFile.input.files.length > 1) {
      label = inputFile.input.files.length + ' файлов'; // multiple selection -> show number of files
    } else {
